...
- (void)loadBlotterDatas
{
    ...
    NSDictionary *dict;
    NSString *url;
    if ([_type  isEqual: @"recharge_price"]) {
        dict = @{@"key":key};
        url=SUGE_BALANCERECORD;
    }else if ([_title isEqualToString:@"账单"]) {
        dict = @{@"key":key};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_type  isEqual: @"recharge_price"]) {
        snLabel.text=[NSString stringWithFormat:@"余额充值,充值单号:%@",BlotterDatas[row][@"pdr_sn"]];
    }else{
        snLabel.text = BlotterDatas[row][@"lg_desc"];}
    
    [cell addSubview:snLabel];
    //时间
    UILabel *timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(snLabel.frame.origin.x+snLabel.frame.size.width-10, 0, SCREEN_WIDTH-snLabel.frame.origin.x-snLabel.frame.size.width-10,20)];
    timeLabel.textAlignment = NSTextAlignmentRight;
    if ([_type  isEqual: @"recharge_price"]) {
        NSString *time= BlotterDatas[row][@"pdr_add_time"];
        ...
        NSString *time= BlotterDatas[row][@"lg_add_time"];
        NSDate *date=[[NSDate alloc]initWithTimeIntervalSince1970:[time intValue]/1000.0];
        timeLabel.text =[NSString stringWithFormat:@"%@",date];
    }
    ...
    if ([_type  isEqual: @"recharge_price"]) {
        wasteLabel.text = BlotterDatas[row][@"pdr_amount"];
    }else{
