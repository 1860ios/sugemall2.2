...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIView *view=[[UIView alloc]initWithFrame:CGRectMake(goodsImageView.frame.origin.x+goodsImageView.frame.size.width-70,goodsImageView.frame.origin.y+goodsImageView.frame.size.height-40,70,35)];
    ...
    UIView *view2=[[UIView alloc]initWithFrame:CGRectMake(goodsImageView.frame.origin.x+5, view.frame.origin.y, goodspriceLabel.frame.size.width+10, goodspriceLabel.frame.size.height)];
    view2.backgroundColor=[UIColor colorWithWhite:0.30 alpha:0.50];
    [cell.contentView addSubview:view2];
    
    UIBezierPath *maskPath1 = [UIBezierPath bezierPathWithRoundedRect:view.bounds byRoundingCorners:UIRectCornerTopLeft cornerRadii:CGSizeMake(3, 3)];
    ...
    cancelButton.frame=CGRectMake(5,0,view.frame.size.width,view.frame.size.height);
    [cancelButton setTitle:@"取消收藏" forState:0];
    cancelButton.titleLabel.font=FONT(14);
    [cancelButton setTitleColor:[UIColor whiteColor] forState:0];
//    cancelButton.layer.cornerRadius=7;
    cancelButton.tag=row;
    [cancelButton addTarget:self action:@selector(cancelFav:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)loadDete:(NSString *)tag
{
    ...
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/html",@"application/json",@"text/json",nil];
    NSDictionary *parameter =@{@"key":key1,@"fav_id":_favorites[[tag intValue]][@"fav_id"]};
