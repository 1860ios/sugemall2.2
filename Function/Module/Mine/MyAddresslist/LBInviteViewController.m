...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UILabel *inviteLabel=[[UILabel alloc]initWithFrame:CGRectMake(imageView.frame.origin.x+imageView.frame.size.width+10,imageView.frame.origin.y+10, 200, 20)];
    ...
    if (indexPath.row==0) {
        imageView.image=IMAGE(@"zhijieyaoqing");
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *text = [NSString stringWithFormat:@"苏格时代"];             //分享内嵌文字
    ...
    [[UMSocialData defaultData].urlResource setResourceType:UMSocialUrlResourceTypeImage url:QRCodeStr];
    //调用快速分享接口
...
-(void)initButton
{
    ...
    [mineButton addTarget:self action:@selector(clickRule) forControlEvents:UIControlEventTouchUpInside];
...
-(void)clickRule
{
    LBRuleViewController *rule=[[LBRuleViewController alloc]init];
