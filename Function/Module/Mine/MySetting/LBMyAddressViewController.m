...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //手机号
    UILabel *telPhone = [[UILabel alloc] initWithFrame:CGRectMake(trueName.frame.origin.x+trueName.frame.size.width, trueName.frame.origin.y, 100, 30)];
    telPhone.adjustsFontSizeToFitWidth = YES;
    telPhone.font = [UIFont boldSystemFontOfSize:16];
    telPhone.text = addressListModel.tel_phone;
    telPhone.textAlignment=NSTextAlignmentLeft;
...
//#warning 收货地址更换传值
    ...
    if (_changeAddress) {
        ...
        [maneger POST:SUGE_CHANGE_ADDRESS parameters:parameter success:^(AFHTTPRequestOperation *op,id responObject){
            ...
            NSDictionary *myUserInfo = @{@"true_name":addressListModel.true_name,@"area_info":addressListModel.area_info,@"tel_phone":addressListModel.tel_phone,@"address":addressListModel.address,@"offpay_hash":_offpay_hash,@"offpay_hash_batch":_offpay_hash_batch,@"allow_offpay":_allow_offpay,@"address_id":k3};
            //通知地址传值
