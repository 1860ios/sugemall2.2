...
- (void)initDetailAddress
{
   ...
   // _t3.placeholder = @"选择区或县";
    ...
    locationButton.frame=CGRectMake(SCREEN_WIDTH/2-80, 10+4*50+15,20, 20);
    [locationButton setImage:IMAGE(@"location") forState:0];
    ...
    UILabel *locationLabel=[[UILabel alloc]initWithFrame:CGRectMake(locationButton.frame.origin.x+locationButton.frame.size.width, locationButton.frame.origin.y-15, 150, 50)];
...
- (void)loadAddressDatas:(NSString *)areaID boo:(NSString *)boo
{
    ...
    [manager POST:SUGE_AREA_LIST parameters:parameter success:^(AFHTTPRequestOperation *op,id responObject){
        ...
        NSLog(@"ere%@",_t1.text);
        //定位时获取id
        if (![_t1.text isEqual:@""]&&![_t2.text isEqual:@""]&&![_t3.text isEqual:@""]&&sId==nil&&shId==nil&&qId==nil) {
            NSString *province;
            NSString *d = [_t1.text substringFromIndex:_t1.text.length-1];
            if ([d isEqualToString:@"省"]) {
              province = [_t1.text substringToIndex:_t1.text.length-1];
            }
            for (int i=0; i<model.area_list.count; i++) {
                modelArea = model.area_list[i];
                NSLog(@"%@",modelArea.area_name);
                if ([province isEqualToString:modelArea.area_name]) {
                    NSLog(@"%@",modelArea.area_id);
                    shId=modelArea.area_id;
                }
            }
            [self loadAddressDatas:shId boo:@"2"];
        }else if(shId!=nil&&sId==nil)
        {
            for (int i=0; i<model.area_list.count; i++) {
                modelArea = model.area_list[i];
                NSLog(@"%@",modelArea.area_name);
                if ([_t2.text isEqualToString:modelArea.area_name]) {
                    NSLog(@"%@",modelArea.area_id);
                    sId=modelArea.area_id;
                }
            }
            [self loadAddressDatas:sId boo:@"2"];
        }else if(sId!=nil)
        {
            for (int i=0; i<model.area_list.count; i++) {
                modelArea = model.area_list[i];
                NSLog(@"%@",modelArea.area_name);
                if ([_t3.text isEqualToString:modelArea.area_name]) {
                    NSLog(@"%@",modelArea.area_id);
                    qId=modelArea.area_id;
                }
            }
        }
        else if(sId==nil&&shId==nil&&qId==nil){
...
//    UITextField *t3 = (UITextField *)[self.view viewWithTag:TEXTFEILD_TAG+2];//手机
    ...
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/html",@"application/json",@"text/json",nil];
        ...
        if (isEdict){
            ...
            parameter=@{@"key":key,@"address_id":addressID,@"true_name":t1.text,@"city_id":sId,@"area_id":qId,@"area_info":address,@"address":_detailText.text,@"tel_phone":t2.text};
            }
        }else{
            if (sId==nil&&shId==nil&&qId==nil) {
        parameter=@{@"key":key,@"true_name":t1.text,@"city_id":id1,@"area_id":id2,@"area_info":address,@"address":_detailText.text,@"tel_phone":t2.text};
            }else{
        parameter=@{@"key":key,@"true_name":t1.text,@"city_id":sId,@"area_id":qId,@"area_info":address,@"address":_detailText.text,@"tel_phone":t2.text};
            }
        }
    [manager POST:url parameters:parameter success:^(AFHTTPRequestOperation *op,id responObject){
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    ...
    [geo reverseGeocodeLocation:location completionHandler:^(NSArray *placemarks, NSError *error) {
        ...
        _t1.text=placemark.addressDictionary[@"State"];//省
        _t2.text=placemark.addressDictionary[@"City"];
        _t3.text=placemark.addressDictionary[@"SubLocality"];
        [self loadAddressDatas:@"" boo:@"1"];
        [SVProgressHUD dismiss];
    }];
}
