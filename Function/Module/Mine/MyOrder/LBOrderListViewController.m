...
{
    if (__orderPay) {
        [__orderPay removeFromSuperview];
    }else{
...
//    }
    __tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-100) style:UITableViewStyleGrouped];
...
//    [str addAttribute:NSForegroundColorAttributeName value:FONT(12) range:NSMakeRange(0,3)];
    ...
    [_refundButton setTitleColor:[UIColor whiteColor] forState:0];
    _refundButton.backgroundColor = APP_COLOR;
    _refundButton.titleLabel.font=FONT(13);
    _refundButton.frame=CGRectMake(SCREEN_WIDTH-10-60,cell.lineView2.frame.origin.y+cell.lineView2.frame.size.height+5,60,25);
    [_refundButton setTitle:@"退 款" forState:0];
    _refundButton.tag = ORDER_BTN_TAG+4;
    [cell addSubview:_refundButton];
    _refundButton.hidden = YES;

    //退款
     if ([_modelList.state_desc isEqualToString:@"已支付"]) {
        _refundButton.hidden=NO;
         cell._searchDeliver.hidden=YES;
    }else if ([_modelList.state_desc isEqualToString:@"待收货"])
    {
        _refundButton.hidden=NO;
...
//    }
    ...
    _refundButton.tag = row+5000;
    [_refundButton addTarget:self action:@selector(refundMethod1:) forControlEvents:UIControlEventTouchUpInside];
