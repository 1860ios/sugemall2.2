...
- (void)addValue: (LBBuyStep1Model *)model
{
    ...
    if (model.address_info.true_name ==NULL) {
        model.address_info.true_name=@"";
    }
    if (model.address_info.mob_phone==NULL) {
        model.address_info.mob_phone=@"";
    }
    if (add==NULL) {
        add=@"";
    }
    _receiverName.text = [NSString stringWithFormat:@"收货人:%@",model.address_info.true_name];
    _receiverPhone.text = [NSString stringWithFormat:@"%@",model.address_info.tel_phone];
