 ...
 *  请求参数
 ...
 vat_hash 发票信息hash
 inv_info 发票信息
 available_predeposit 可用佣金
 available_rc_balance 可用余额
...
/*
 ...
 key 用户key
 pagesize 每页展示我的客户数量
 curpage 当前页数（从第1页开始curpage=1）
 返回数据
 
 custom_list 我的客户
 custom_list.member_avatar （我的客户）头像
 custom_list.member_name （我的客户）昵称
 custom_list.contribution （我的客户）贡献值
 custom_list.count_teams （我的客户）团队值
 */
...
/*
 ...
 partner_list 合伙人
 partner_list.member_avatar （合伙人）头像
 partner_list.member_name （合伙人）昵称
 partner_list.contribution （合伙人）贡献值
 partner_list.count_teams （合伙人）团队值
 */
...
/**
 POST数据：
 key:用户key值
 pay_sn：充值订单支付单号（传此参数则返回对应pay_sn的充值订单，不传此参数则直接拿此用户的所有充值订单列表记录）
 
 返回数据：
 pdr_id:编号
 pay_sn：充值订单支付单号
 pdr_member_id:用户member_id
 pdr_member_name
 pdr_amount:充值金额
 pdr_add_time:增加时间
 pdr_payment_state:支付状态 0未支付 1成功支付
 pdr_payment_time:支付时间
 */
#define SUGE_BALANCERECORD             [SUGE_BASE_URL stringByAppendingString:@"index.php?act=predeposit&op=index"]

//<<<<<<< HEAD
#define SUGE_MYREDBAG             [SUGE_BASE_URL stringByAppendingString:@"index.php?act=member_index&op=bonus"]
/*
 传参：POST
 key :  用户key
 返回数据：
 未实名认证：
 id=0（如果用户未经过实名认证，则仅返回id=0）
 已实名认证：
 id
 ...
 member_mobile
 */
#define SUGE_REALNAME             [SUGE_BASE_URL stringByAppendingString:@"index.php?act=member_security&op=authinfo"]
/**请求参数
 ...
 pay_sn (成功)充值订单支付单号
 error （失败）错误信息*/
#define SUGE_ADDRECHARGE             [SUGE_BASE_URL stringByAppendingString:@"index.php?act=predeposit&op=recharge_add"]
/*
 请求参数
 ...
 html格式的文章内容
 */
#define SUGE_PARTNERRULE             [SUGE_BASE_URL stringByAppendingString:@"index.php?act=suge_article&op=getcontent"]
